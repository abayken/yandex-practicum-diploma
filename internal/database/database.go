...
func (storage *DatabaseStorage) initTablesIfNeeded() {
	...
	_, err = storage.DB.Exec(context.Background(), "CREATE TABLE IF NOT EXISTS TRANSACTIONS (ID SERIAL PRIMARY KEY, USER_ID INTEGER, ORDER_NUMBER VARCHAR(100), SUM BIGINT, ADDED_AT TIMESTAMP DEFAULT NOW());")
