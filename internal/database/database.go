...
func (storage *DatabaseStorage) initTablesIfNeeded() {
	...
	_, err = storage.DB.Exec(context.Background(), "create table if not exists orders (id serial primary key, user_id integer, status varchar(100), number varchar(100) unique, added_at timestamp default now());")
